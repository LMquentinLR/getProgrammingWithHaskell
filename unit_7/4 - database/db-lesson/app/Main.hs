...
class FromRow a where
  fromRow :: F.RowParser a
instance F.FromRow User where
  fromRow = User <$> field
                 <*> field
instance F.FromRow Tool where
  fromRow = Tool <$> field
                 <*> field
                 <*> field
                 <*> field
...
insertCheckoutQuery :: Query
insertCheckoutQuery = "INSERT INTO checkedout VALUES (?,?)"

selectToolQuery :: Query
selectToolQuery = "SELECT * FROM tools WHERE id = (?)"

checkinQuery :: Query
checkinQuery = "DELETE FROM checkedout WHERE tool_id = (?);"
-- FUNCTIONS

--oldAddUser :: String -> IO ()
--oldAddUser userName = do
--  conn <- open "tools.db"
...
addUser userName = withConn "tools.db" $ 
  \conn -> do
    execute conn insertQuery (Only userName)
    print "user added"

checkout :: Int -> Int -> IO ()
checkout userId toolId = withConn "tools.db" $ 
  \conn -> do
    execute conn insertCheckoutQuery (userId, toolId)
...
checkIn  toolId = withConn "tools.db" $
  \conn -> do
    execute conn checkinQuery (Only toolId)
    print "checkin performed"
