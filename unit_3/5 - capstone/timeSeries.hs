...
createTS :: [Int] -> [a] -> TS a
--want to create time series w/ a full timeline and a list of Maybe values
--assume the arguments may represent only a limited set of possible values
createTS times values = TS completeTimes extendedValues
--completeTimes are all the times from the minimum up to the maximum
  where completeTimes = [minimum times .. maximum times]
  --create a simple Map of the times and values you know you have
        timeValueMap = Map.fromList (zip times values)
        --mapping lookup over complete times, you'll get Just x values for all
        --existing values and Nothing for all missing values. This takes care of 
        --filling in the complete set of values to match the complete timeline
